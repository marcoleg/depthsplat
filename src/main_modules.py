...
@dataclass
class DepthSplatSession:
    cfg_dict: DictConfig
    cfg: object
    eval_cfg: Optional[object]
    output_dir: Path
    trainer: Trainer
...
def create_depthsplat_session(cfg_dict: DictConfig) -> DepthSplatSession:
...
def load_pretrained_weights(session: DepthSplatSession, stage: str) -> None:
    cfg = session.cfg
    strict_load = session.strict_load
    if stage == "train":
        if cfg.checkpointing.pretrained_monodepth is not None:
            ...
            session.model_wrapper.encoder.depth_predictor.load_state_dict(
        ...
        if cfg.checkpointing.pretrained_mvdepth is not None:
            pretrained_model = torch.load(
                cfg.checkpointing.pretrained_mvdepth, map_location="cpu"
            )["model"]
            ...
            print(
                cyan(
                    f"Loaded pretrained mvdepth: {cfg.checkpointing.pretrained_mvdepth}" # type: ignore
                )
            )
    ...
    if cfg.checkpointing.pretrained_model is not None:
        ...
        if "state_dict" in pretrained_model:
            pretrained_model = pretrained_model["state_dict"]
        session.model_wrapper.load_state_dict(pretrained_model, strict=strict_load)
        print(
            cyan(
    ...
    if cfg.checkpointing.pretrained_depth is not None:
        pretrained_model = torch.load(
            cfg.checkpointing.pretrained_depth, map_location="cpu"
        )["model"]
    ...
    session.strict_load = strict_load

def run_training(session: DepthSplatSession) -> None:
    load_pretrained_weights(session, stage="train")
    session.trainer.fit(
...
def train(cfg_dict: DictConfig):
    session = create_depthsplat_session(cfg_dict)
    if session.cfg.mode == "train":
